...
main(void)
{
    ...
    create_shader(&sh,
              "/home/joe/Programming/c/learn_opengl/ch06/shaders/circ_color.vs",
              "/home/joe/Programming/c/learn_opengl/ch06/shaders/circ_color.fs");
    ...
    while (!glfwWindowShouldClose(window)) {
        ...
        h_offset = cos(time_value) / 2.0f;
        v_offset = sin(time_value) / 2.0f;

        r_val = (cos(time_value) / 2.0f) + 0.5f;
        g_val = (cos(time_value + M_PI_2) / 2.0f) + 0.5f;
        b_val = (cos(time_value + M_PI) / 2.0f) + 0.5f;

        offset_location = glGetUniformLocation(sh.ID, "offset");
        color_location = glGetUniformLocation(sh.ID, "color");

        glUniform2f(offset_location, h_offset, v_offset);
        glUniform3f(color_location, r_val, g_val, b_val);
